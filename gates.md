...
> **阈值以 `reference/escalation-redlines.md` R2 为单一真相源（SSOT）**：must-fix **> 3 项**（即 ≥ 4 项）触发，本处不再独立定义，避免两处 `≥3` / `>3` 打架。

**触发条件**：
- Phase 9 安全审查（任一路：自审 / 对抗审 / 安全审）发现 **> 3 项**（即 ≥ 4 项）**阻塞发布**的 must-fix
- 或：1 项极严重（CVSS ≥ 9）
