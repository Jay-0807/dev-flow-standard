...
- `iteration-vault/01.5-user-research.md`（**🆕 v2 必读** — 3 个用户画像 + 痛点 ranking + 用户心智隐形 5 字段。若 Phase 1.5 被跳过，PRD 第 3、3.5、4、7.4 节会显性标"假设性"待 12.5 早晨复盘真人验收）
- `<project-context>.md`（项目自定，含隐形信息显性化要求 / 合规约束如有，**v2 含用户心智维度**）
- `<project-positioning>.md`（项目自定，含项目合规要求约束如有）
...
**适配重点**：调用前先把 PRD 中的产品与目标用户上下文（如有）喂给它，让它以该领域为前提思考，而不是泛泛而谈。

输入给 /autodev-ideation 的前置 prompt：
```
背景：本次需求服务于 [PRD 中的产品与目标用户]。
约束：按启动时检测到的项目实际技术栈；项目合规要求（如有）要求隐形信息显性化。
请基于该领域思考本需求的：
- 产品形态可能性（短 / 中 / 长 3 个版本）
- 与同类竞品的差异点
- 目标用户最在意的功能切片
...
3. **目标用户 & 场景**：**强制引用** 01.5-user-research.md 的 3 个画像（不允许 PM 凭空写角色名，须引用 01.5 画像）
...
7. **隐形信息显性化**（**四维**强制段）：
   ...
   - 7.2 人工保留点
   - 7.3 数据来源 & 置信度
   - 7.4 **🆕 用户心智隐形**（5 字段，来自 01.5）：通用的"用户没说但在意 / 上次不爽场景 / 决策暗信号"对任何产品都适用；其中"用户对 AI 的期待 / AI 微表情"等 **AI 期待相关子字段（仅 AI 原生项目）**，非 AI 项目可填 N/A 或跳过
8. **指标 & 度量**：怎么知道做对了？
9. **风险 & 依赖**：技术/合规/业务三类
...
**PM 选 ✅**：直接进入 Phase 2.5 brainstorm。
