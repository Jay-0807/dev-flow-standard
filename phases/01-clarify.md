...
> 内部编号：Phase 1（含原 Phase 0 需求接收 — 本文件已合并）
> 内部分两步：① 接收 PM 原话 + 大小分级 → ② 扩展为正式需求 1-pager
> 模式：💬 PM 在线对话
...
**Step 0（前置）：经验召回 —— 开工前先翻历史，闭合跨迭代学习回路**
在复述需求前先消除"每轮失忆、重复踩坑"：
1. Read 项目根 `CLAUDE.md`（若有）—— 耐久铁律 / 已知坑
2. Grep `iteration-vault/*/findings.md` + `iteration-vault/*/autonomous-decisions.md` + 自动记忆，匹配本次需求领域关键词（如"扫描器 / 上传 / 表单 / 批量生成"）
3. 命中旧教训 → 写进 Step 1 复述（"上次在这块学到 X，本次沿用 / 避免 Y"），并作为强约束带进后续 phase
4. 无命中 → 一句话说明"未发现相关历史经验"，继续
> 配套收尾：Phase 12.5 / 13 把本轮耐久教训回写 CLAUDE.md + PRD + 自动记忆，否则下轮又失忆。
