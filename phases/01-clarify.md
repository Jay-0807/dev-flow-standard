...
**Step 4.5：④ 盲区探针（非快车道强制）—— 挖"不知道自己不知道"**
基于未知四象限（见 `principles/karpathy-llm-coding.md` 原则 1 操作工具）：Step 4 的 AskUserQuestion 处理的是 ②③（已知没写 / 伪已决），本步补上 ④（真盲区）。
- 主动列出"这个需求最可能翻车的 3 个点"（如：依赖的外部数据格式没见过 / 并发或边界情形没想过 / 与存量功能的隐藏耦合）
- 每个探针配一个"先验证动作"（读一段代码 / 查一个接口 / 跑一个最小实验），写进产出的"隐性信息"段
- 🏃 快车道可省（一行自检即可）；🚶🐢 或命中风险面**强制做**

**Step 5：注入项目业务上下文（如有）反问**
Read `<project-business-context>.md（universal 版无内置业务上下文，可由项目自定）`，按其中的"隐形信息检查清单"主动反问 PM：
