...
- `<project-business-context>.md（universal 版无 项目业务哲学（如有），可由项目自定）`（**重点**：第 4 维"用户心智隐形"——错误提示文案、loading 反馈、置信度可视化）
...
```mermaid
sequenceDiagram
  店主->>首页: 看到顶部入口"AI 选品"  [屏 1]
  店主->>屏1: 点入口
  屏1-->>店主: 跳转到选品列表  [屏 2]
  店主->>屏2: 看 AI 推荐 + 置信度  [关键: 置信度必须可见]
  店主->>屏2: 选 3 个
  店主->>屏2: 点"加入计划"
  屏2-->>店主: 弹窗确认  [屏 3]
```
...
- 只有方框 + 文字标签 + 主要按钮
- 标注每个区域是什么（"这里放 AI 推荐列表"）
- 标注关键交互（"点这个按钮 → 弹这个窗"）

涉及屏：[屏 1 简述, 屏 2 简述, 屏 3 简述]

设计约束（项目特定）：
- 必须显性显示 AI 置信度（参考 r4 第 4 维：用户对 AI 的真实期待）
- 必须有"人工介入"入口（参考 r4 第 6 字段：人工保留点）
- 错误提示要解释"为什么+怎么办"，不只是 error code
```
...
### 1.5 r4 第 4 维度用户心智隐形检查

回看 01.5 文件的"用户心智隐形信息"段：

- "用户没说但在意"的点 → 是否在 wireframe 中体现？
- "用户上次不爽场景" → 新设计是否避免了这个场景？
- "用户对 AI 的真实期待" → 置信度/可控性是否可视化？
...
设计约束：必须显性显示 AI 置信度（参考 §1 文档中 r4 第 4 维检查）
```
...
|---|---|---|---|---|
| AIRecommendationCard | 新增 | 屏 2 推荐项卡片 | item, confidence, onSelect | 含置信度可视 |
| Button | 复用 | 通用 | shadcn/ui 默认 | - |

| Token | 类型 | 值 | 新增/复用 |
|---|---|---|---|
| color-confidence-high | 颜色 | #10B981 | 新增 |
...
| §1 决定 | §2 是否落地 | 落地位置 |
|---|---|---|
| 屏 2 显性显示置信度 | ✅ | AIRecommendationCard 组件的 confidence prop |
