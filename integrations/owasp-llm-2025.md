...
**项目适配点**：
- 项目业务协议（如有）消息也可能是注入向量（agent A 给 agent B 的消息可能含恶意指令）
- 客户上传的访谈 / 文档喂给 LLM 前要做 sanitize
...
**项目适配点**：
- 项目业务协议（如有）下 agent 之间互信问题（你怎么知道对面 agent 没被替换）
...
**项目适配点**：
- 萤火虫从访谈中提取的规则若直接喂入业务 LLM，需要中间审核
...
**项目适配点（关键！）**：
- 萤火虫合同 9.1 "AI 仅作建议"原则要求：高风险决策必须人工确认
- 合同 A1.4 六类人工保留事项 = 这一项的具体红线
...
**项目适配点**：
- r4 哲学要求：每个 AI 决策都要标"置信度 + 数据来源"
- 合同 9.1 要求：AI 仅作建议，不能替代人类判断
